...
| みんなの写真と声（利用者の投稿） | 投稿しだい | 利用者が投稿（受け皿: 運営者の Google ドライブ／スプレッドシート・`tools/posts_api.gs`） | 著作権は投稿者に帰属。本サイトでの表示に使う | 投稿者名を表示。通報 3 件で自動非表示・運営者がシートで非表示にできる。公開する写真は縮小・撮影データ（EXIF）なし。撮影位置・日時・機種・投稿時の現在地は運営者だけが見られるシート «ExifLog»（管理画面 `admin/posts.html`）に残し、公開しない |
...
## v86
- 書体 — Google Fonts（Plus Jakarta Sans: SIL OFL 1.1／Noto Sans JP: SIL OFL 1.1／JetBrains Mono: SIL OFL 1.1）。端末から fonts.googleapis.com・fonts.gstatic.com へ直接取得する（Google のプライバシーポリシーに従う）。
