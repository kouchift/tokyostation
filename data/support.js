/* 制作者への窓口（投げ銭・改善要望）の設定  v71$301C
   ここだけ書き換えれば、コードを触らずに窓口を変えられます。

   paypayId / kyashId : 送金先のアカウント名（画面に出して、コピーできるようにする）
   paypayLink / kyashLink : $00AB送り先入り$00BB でアプリが開くリンク。貼ると、金額ボタン 1 タップでアプリが送金画面まで開く（v87）。
       PayPay: アプリのホーム →「受け取る」→ マイコード →「共有」→ リンクをコピー（https://qr.paypay.ne.jp/… の形）。
               ※「送金リンク（受け取りリンク）」は $00AB自分が誰かに送る$00BB ための 4 日で切れるリンクなので、ここには使えない。
       Kyash : 「請求」→ 金額を入れずに請求リンクを作成 → コピー。
       空のときは「ID をコピー → アプリのトップを開く」になり、アプリ側で「送る → ID を貼る」の 2 手が残る
       （アカウント名だけで開ける公開 URL は両社とも用意していないため）。
   paypayLinks / kyashLinks : 金額ごとのリンク {100: "…", 500: "…"}。PayPay はマイコードで $00ABリクエスト金額$00BB を入れてから共有、
       Kyash は請求リンクに金額を入れて作ると、相手は金額も入力せずに $00AB送る$00BB を押すだけになる（最短）。無い金額は paypayLink に落ちる。
   paypayQr / kyashQr : PC 画面に出す $00ABマイコード$00BB の画像（例 "assets/img/paypay_qr.png"）。スマホのカメラ／アプリで読むと送り先入りで開く。
       空のときは、このサイトの $00AB押すだけ$00BB 画面を開く QR（スマホ側は 1 タップ）を出す。
   ...
   BANLIST : アクセスを断つ来訪者ID（メッセージの末尾に付く vid:xxxx の xxxx）。
             ※ 静的サイトなので $00ABその端末のブラウザ$00BB に対する拒否にとどまる（別の端末・シークレットモードでは無効）。
...
RG.TIP = {
  ...
  /* 現場メモ（v79）: 受け皿を用意すると $00ABみんなのメモ$00BB になる（無くても端末内で動く）
  ...
  memoCsv: "",
  /* みんなのコメント（v106）: Google Apps Script のウェブアプリ URL（tools/comments_api.gs。README「コメント管理者向け設定」）。空ならスポットカードは従来の $00AB行った人の声$00BB */
  commentsApi: "https://script.google.com/macros/d/14C6S1nFYZl5fzanqTUy2x4ZQZf0wqYLjITSJyIAY-L63gJW0R5ObSAEa/usercopy",
  /* イイね！（v85）: 別の Google フォーム（列: id,vid,t,name）。id は $00AB投稿者の印:投稿時刻$00BB で投稿と結びつく。likeCsv はその回答シートの CSV */
  likeForm: { action: "", fields: { id: "", vid: "", t: "", name: "" } },
  likeCsv: "",
  /* 閲覧（v85、任意）: 場所ごとの閲覧者数を $00ABみんな$00BB で数えたいときだけ（列: name,vid,t。1 端末 1 日 1 回）。無ければ投稿・イイね！した人＋自分だけで数える */
  ...
  /* v100: 受取先の設定（公開してよい識別子だけ。パスワード・認証コード・口座番号・電話番号は絶対に書かない）
     ・link / qr が空のサービスは $00AB準備待ち$00BB として表示し、架空の URL は作らない。期限つきの受け取りリンク（楽天ペイ 3 日・請求リンク 2 週間など）は貼らない
     ・email は $00AB受取アカウントの情報$00BB であって、メールアドレス宛に送れるかはサービスごとに違う（PayPal・ことら送金は可、楽天ペイ・d払い・au PAY は不可＝公式ガイド 2026-09 確認）
     ・paypal.paypalMe を入れると PayPal.Me のボタンが出る（例 "tonbo7" → https://www.paypal.com/paypalme/tonbo7）。推測では作らない
     ・coinplus.userNumber は エアウォレット等の対応アプリで $00AB送金 → ユーザー番号$00BB に入れる識別子。ブラウザから自動送金できるものではない
     ・famipay.enabled は、FamiPay 残高の個人間送金が公式に確認できたら true に（それまでは $00AB対象外$00BB と表示）
     ・cotra（ことら送金）: 送金する人は対応する銀行アプリから $00ABメールアドレス$00BB を指定。受け取る側は銀行アプリで $00ABメールアドレスと口座の紐付け（受取設定）$00BB が必要。
       registered を false にすると「受取設定が必要です」と表示して送金ボタンを出さない。電話番号は使わない（公開しない） */
  services: {
    rakutenpay: { email: "tonbo7@gmail.com", link: "", qr: "" },
    dbarai:     { email: "tonbo7@gmail.com", number: "", link: "", qr: "" },           // number = d払い番号（公開してよい ID。あれば $00AB送れる$00BB に）
